...
BFaBFb.oneDesign.random <- function(
  ...
  m.a = std.a; m.b=std.b; m.cp=std.cp
  ## consider the standardized simple mediation model
  ## consider var_em and var_ey have no uncertainty
  var_em=1-m.a^2; var_ey=1 - m.cp^2 - m.b^2 - 2*m.a*m.b*m.cp
  
  bfabfb.samplingdist.random <- function(N, m.a1, sigma.a1,
                             m.b1, sigma.b1, 
                             m.cp, sigma.cp,
    ...
    OneData.cond.random <- function(i=1, N, 
                             m.a1, sigma.a1,
                             m.b1, sigma.b1, 
                             m.cp, sigma.cp
                             ){
      # randomly draw a set of parameter values from the design prior distribution
...
BFmed.oneDesign <- function(
    bfabfb_pools.list, Design.PriorOdds.a, Design.PriorOdds.b
    ) {
  ...
  Nsim_H0=Nsim_H1=R ; Nplan=N; 
  
  asize= a; bsize= b; cpsize= cp
...
BFmed.oneAnalysis <- function( 
    bfmed_Nsim_H0H1.list, Analysis.PriorOdds.a, Analysis.PriorOdds.b,
  ...
  if( relative.cutoff==TRUE ){
    ...
    TPR_bfmed = power_bfmed = sapply(cutoff_bfmed, function(x){mean( bfmed_Nsim_H0H1$BFmed[bfmed_Nsim_H0H1$trueH=='H1'] > x )})  # true positive rate
  ...
  if( absolute.cutoff == TRUE ){
    ...
    FPR_bfmed_abs = TypeI_bfmed_abs = sapply(cutoff_abs, function(x){mean( 
      bfmed_Nsim_H0H1$BFmed[bfmed_Nsim_H0H1$trueH=='H0'] > x )})
    
    TPR_bfmed_abs = power_bfmed_abs = sapply(cutoff_abs, function(x){mean(
      bfmed_Nsim_H0H1$BFmed[bfmed_Nsim_H0H1$trueH=='H1'] > x )})
  ...
  ## output 
  designprior=c(Design.PriorOdds.a=Design.PriorOdds.a, Design.PriorOdds.b=Design.PriorOdds.b, 
                Design.q00=Design.q00, Design.q10=Design.q10, Design.q01=Design.q01)
  analysisprior = c(Analysis.PriorOdds.a=Analysis.PriorOdds.a, Analysis.PriorOdds.b=Analysis.PriorOdds.b, 
                    Analysis.q00=Analysis.q00, Analysis.q10=Analysis.q10, Analysis.q01=Analysis.q01)
  N=Nplan; a=asize;b=bsize;cp=cpsize;
...
BFmed.SSD <- function(
  ...
  if( uncertain.effect == FALSE ){
    mcmapply(BFaBFb.oneDesign, N=datacond$N, 
           std.a=datacond$std.a, std.b=datacond$std.b,std.cp=datacond$std.cp, MoreArgs = list( R=R, seed=seed), SIMPLIFY = F, mc.preschedule = T,
           mc.cores = 1
           ) -> bfabfb_pools
  }
  ...
  if( uncertain.effect == TRUE ){
    ...
    mcmapply(BFaBFb.oneDesign.random, N=datacond$N, 
           std.a=datacond$std.a, std.b=datacond$std.b,std.cp=datacond$std.cp, 
           sigma.a=datacond$sigma.a, sigma.b=datacond$sigma.b,sigma.cp=datacond$sigma.cp, 
           MoreArgs = list( R=R, seed=seed), SIMPLIFY = F, mc.preschedule = T,
           mc.cores = 1
           ) -> bfabfb_pools
  }
...
TPR.plot = ggplot( plotdf1
 ...
 facet_grid( Design.PriorOdds.a + Design.PriorOdds.b ~ a+b,  labeller = label_both ) +
  ...
  geom_point( aes(x=N,y=true_positive_bfmed,
                  shape=Analysis.PriorOdds  ),size=2.5 ) +
  geom_line( aes(x=N, y=true_positive_bfmed
                 ,color= "relative to the specified false positive rate,\n cutoff.FPR"
                # , color=as.factor(round(PriorOdds.b,2))
  ) ) +
  geom_point( aes(x=N,y=true_positive_bfmed_abs,
                  shape=Analysis.PriorOdds  ),size=2.5 ) +
  geom_line( aes(x=N, y=true_positive_bfmed_abs
                 ,color= "the specified absolute cutoff,\n cutoff.BF"
                 #, color=as.factor(round(PriorOdds.b,2))
  ) ) +
...
plot.false.positive.med <- function( SSD_BFmed){
 ...
 facet_grid( Design.PriorOdds.a + Design.PriorOdds.b ~ a+b,  labeller = label_both ) +
  ...
  geom_point( aes(x=N,y=cutoff.FPR,
                  shape=Analysis.PriorOdds  ),size=2.5 ) +
  geom_line( aes(x=N, y=cutoff.FPR
                 ,color= "relative to the specified false positive rate,\n cutoff.FPR"
                # , color=as.factor(round(PriorOdds.b,2))
  ) ) +
  geom_point( aes(x=N,y=false_positive_abs,
                  shape=Analysis.PriorOdds  ),size=2.5 ) +
  geom_line( aes(x=N, y=false_positive_abs
                 ,color= "the specified absolute cutoff,\n cutoff.BF"
                 #, color=as.factor(round(PriorOdds.b,2))
  ) ) +
